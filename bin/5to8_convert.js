...
//var f5to8 = fs.readFileSync('C:\\Users\\win10\\Downloads\\subway_data\\2014_5TO8.csv', 'utf8');
var f5to8 = fs.readFileSync('/home/jongsang/local/2014_5TO8.csv', 'utf8');
parse(f5to8, {comment:"#"}, function(csv_err, csv_data){
  if (csv_err) {
  ...
  for(var cd=1; cd< csv_data.length ; cd+=2){
    ...
    if(dataIn[0]===dataOut[0] && dataIn[1]===dataOut[1] && dataIn[2]===dataOut[2]){
      ...
      if(!s_meta[lStationName]){
        station_name = sDiffNames[lStationName] && sDiffNames[lStationName][0];
...
//    	  var ldate = new Date(ldateTemp[0],Number(ldateTemp[1]),ldateTemp[2],h);
        ...
        var s_logs = {
          ...
          "station_name_kr" : s_meta[station_name] && s_meta[station_name].STN_NM_KOR,
          "station_name_en" : s_meta[station_name] && s_meta[station_name].STN_NM_ENG,
          "station_name_chc" : s_meta[station_name] && s_meta[station_name].STN_NM_CHC,
          "station_name_ch" : s_meta[station_name] && s_meta[station_name].STN_NM_CHN,
          "station_name_jp" : s_meta[station_name] && s_meta[station_name].STN_NM_JPN,
          "station_geo" : {
            "lat" : s_meta[station_name] && s_meta[station_name].GEO_X,
            "lon" : s_meta[station_name] && s_meta[station_name].GEO_Y
          },
          "people_in" : people_in,
